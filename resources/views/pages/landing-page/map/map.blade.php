        ...
        <div class="wrap m-auto min-h-[95vh] flex md:flex-row flex-col gap-3 pt-[100px] px-3 text-black">
            ...
            <div class="right order-1 md:order-2 w-full md:w-[75%] flex flex-col gap-4">
                ...
                <div class="suges h-[130px] md:h-[20%] w-full flex gap-3 overflow-auto">
                    <div class="dokter1 flex-shrink-0  w-[310px] bg-white border-[#BCB3A3] border p-3 flex gap-3 rounded-lg shadow-lg" popup-dokter-class="marker30" style="cursor: pointer">
    ...
    <script>
        ...
        const loadLocations = () => {
            geoJson.features.forEach((location) => {
                ...
                const contents = `
                <div class="dokter1  md:w-[310px] w-[260px] bg-[#84A584] border-[#BCB3A3] border p-3 flex gap-3 rounded-lg shadow-lg">
                    ...
                    <div class="detail flex flex-col justify-between text-white">
                        <div class="wrap flex flex-col gap-1">
                            ...
                            <div class="no md:text-[11px] text-[10px] ">
                                ${sip}
                            </div>
                        </div>
                        <div class="wrap gap-3 flex items-center">
                            ...
                            <a data-long="${geometry.coordinates[0]}" data-lat="${geometry.coordinates[1]}" onclick="lihatTutupRute(this);" href="#"  class="p-1 bg-white text-[10.5px] text-black rounded">
                                Lihat Rute
                            </a>
                        </div>
                    </div>
                ...
                <div class="detail  text-black min-h-[70vh]">
                        ...
                        <div class="wrap-teks mt-3 flex flex-col gap-1">
                            ...
                            <div class="teks flex gap-1">
                                ...
                                <div class="ka" style="font-size: 80%;">${sip}</div>
                            </div>
                            <div class="teks flex gap-1">
        ...
        var geolocatecontrol = new mapboxgl.GeolocateControl({
            ...
            // fitBoundsOptions: {maxZoom:11.15}
        });
        ...
        function lihatTutupRute(event) {
            const directionProfile = document.querySelector(".mapbox-directions-profile.mapbox-directions-component-keyline.mapbox-directions-clearfix");
            const buttonHideDirection = document.querySelector("button[id=mapboxgl-ctrl-hide-directions]");
            directions.setOrigin([geolocatecontrol._accuracyCircleMarker._lngLat.lng, geolocatecontrol._accuracyCircleMarker._lngLat.lat]);
            if (event.innerText === "Tutup Rute") {
                directions.removeRoutes();
                directionProfile.setAttribute("style", "display: none;");
                buttonHideDirection.setAttribute("style", "display: none;");
                event.innerText = "Lihat Rute";
                return
            }
            long = event.getAttribute("data-long");
            lat = event.getAttribute("data-lat");
            directions.setDestination([long, lat]);
            directionProfile.setAttribute("style", "display: block;");
            buttonHideDirection.setAttribute("style", "");

            if (event.innerText === "Lihat Rute") {
                event.innerText = "Tutup Rute";
            } else {
                event.innerText = "Lihat Rute";
            }
        }
        ...
        function hideDirection(event) {
            const buttonHideDirection = document.querySelector("button[id=mapboxgl-ctrl-hide-directions]");
            const buttonShowDirection = document.querySelector("button[id=mapboxgl-ctrl-show-directions]");
            buttonHideDirection.setAttribute("style", "display: none;")
            buttonShowDirection.setAttribute("style", "")
            const directionControl = document.querySelector(".mapboxgl-ctrl-directions.mapboxgl-ctrl");
            directionControl.setAttribute("style", "display: none;")
        }
        ...
        function showDirection(event) {
            ...
            const buttonShowDirection = document.querySelector("button[id=mapboxgl-ctrl-show-directions]");
            buttonHideDirection.setAttribute("style", "")
            buttonShowDirection.setAttribute("style", "display: none;")
            const directionControl = document.querySelector(".mapboxgl-ctrl-directions.mapboxgl-ctrl");
            directionControl.setAttribute("style", "display: block;")
        }
        ...
        const controlLeft = document.querySelector("div[class=mapboxgl-ctrl-top-left]");
        let customControlLeftElem = document.createElement("div");
        customControlLeftElem.setAttribute("class", "mapboxgl-ctrl mapboxgl-ctrl-group mapboxgl-ctrl-show-hide");
        customControlLeftElem.innerHTML = '<button id="mapboxgl-ctrl-show-directions" class="mapboxgl-ctrl-geolocate mapboxgl-ctrl-geolocate-background" type="button" style="display: none;" onclick="showDirection(this);"><svg class="w-2.5 h-2.5 ms-2.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 6"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"></path></svg></button><button id="mapboxgl-ctrl-hide-directions" class="mapboxgl-ctrl-geolocate mapboxgl-ctrl-geolocate-background" type="button" style="display: none;" onclick="hideDirection(this);"><svg class="w-2.5 h-2.5 ms-2.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 6"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m9 5 -4 -4 -4 4"></path></svg></button>';
        controlLeft.append(customControlLeftElem);

        const doktersOtherHistory = document.querySelectorAll(".dokter1.flex-shrink-0");
        doktersOtherHistory.forEach(dokterOtherHistory => {
            dokterOtherHistory.addEventListener('click', (event) => {
                for (let dokterEvent of event.composedPath()) {
                    if (dokterEvent.classList.contains("dokter1")) {
                        const popupDokter = document.querySelector("." + dokterEvent.getAttribute("popup-dokter-class"))
                        setTimeout(function () {
                            popupDokter.click();
                        }, 500);
                        break;
